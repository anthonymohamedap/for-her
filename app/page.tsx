...
import MiddleZone from '@/components/garden/MiddleZone'
...
// ─── Parallax mouse hook (desktop only) ─────────────────────────────────────
function useParallax(isDesktop: boolean) {
  ...
  // Layer speeds: bg=0.1, mid=0.3, fg=0.6
  const bgX = useTransform(springX, v => v * 0.1)
  const bgY = useTransform(springY, v => v * 0.1)
  const midX = useTransform(springX, v => v * 0.3)
  const midY = useTransform(springY, v => v * 0.3)
  const fgX = useTransform(springX, v => v * 0.6)
  const fgY = useTransform(springY, v => v * 0.6)
  ...
  useEffect(() => {
    ...
    function onMove(e: MouseEvent) {
      ...
      // Clamp to ±30px range
      rawX.set(Math.max(-30, Math.min(30, e.clientX - cx)))
      rawY.set(Math.max(-30, Math.min(30, e.clientY - cy)))
    }
...
export default function GardenPage() {
  ...
  const { bgX, bgY, midX, midY, fgX, fgY } = useParallax(isDesktop)
  ...
  // Responsive sizing
  // ── Yeon (left character) ─────────────────────────────────────────
  const yeonHeightVh = isDesktop ? 42 : 28
  const yeonWidthVw = isDesktop ? 20 : 28
  // ── Anthony (right character) — adjust these to resize ────────────
  const anthonyHeightVh = isDesktop ? 40 : 35   // ← change these values to resize Anthony
  const anthonyWidthVw = isDesktop ? 18 : 35   // ← desktop: vh/vw, mobile: second value
  ...
  return (
    <main className="relative w-full h-screen overflow-hidden bg-black">
      ...
      >
        ...
        </motion.div>{/* end midground parallax layer */}

        {/* ── Foreground layer — parallax 0.6× ─────────────────────────── */}
        ...
        </motion.div>{/* end foreground parallax layer */}
        ...
        >
          <Character
            ...
            alt="Anthony"
            heightVh={anthonyHeightVh}
            widthVw={anthonyWidthVw}
            floatDelay={0.9}
            entranceDelay={3.8}
            entranceFrom="right"
            breatheDuration={3.8}
        ...
        {/* ── Scene vignette (desktop) — frames the world ──────────────── */}
        <div
          className="absolute inset-0 pointer-events-none hidden md:block"
          style={{
            zIndex: 45,
            background: 'radial-gradient(ellipse at center, transparent 55%, rgba(2,1,10,0.72) 100%)',
          }}
        />

        {/* ── Middle content zone (presence + streak + CTA) ─────────── */}
        <MiddleZone openingDone={openingDone} />

        {/* ── Spotify widget (mobile only) ──────────────────────────── */}
        <SpotifyWidget />
      </motion.div>
    </main>
  )
